 ...
 *
 * Previously hit ENETUNREACH connecting to Gmail over IPv6 on Railway, and
 * a manual IPv4-resolution workaround here was only partially reliable
 * (one resolved address timed out with no fallback that didn't reintroduce
 * the same IPv6 problem). Root cause turned out to be Railway's per-service
 * "Outbound IPv6" networking setting defaulting to off — enabling it fixed
 * delivery outright, so the plain, standard nodemailer config below is
 * correct as-is; no IP-resolution workaround needed.
 */
